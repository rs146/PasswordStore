...
public class ChangePasswordActivity extends Activity{
	    ...
	    if (passwordCheck){
	    	if (password.equals(password2)){
	    		if (!password.isEmpty()){
	    			...
	    			db.close();
	    			Toast.makeText(getBaseContext(), "Thank you. Your Password has been changed", Toast.LENGTH_LONG).show();
            		Intent i = new Intent("com.blackcrowsys.DisplayTableActivity");
            		startActivity(i);
	    		} else {
	    			Toast.makeText(getBaseContext(), "Please enter a Password value", Toast.LENGTH_LONG).show();
	    		}
	    	} else {
	    		Toast.makeText(getBaseContext(), "The New Passwords do not match", Toast.LENGTH_LONG).show();
	    	}
	    	
	    } else {
	    	Toast.makeText(getBaseContext(), "The Current Password has been entered incorrectly", Toast.LENGTH_LONG).show();
	    }	
	}
	...
	public boolean checkOldPassword(String encryptedPass){
		...
		if (this.getUsername().equals(c.getString(0)) && encryptedPass.equals(c.getString(1))){
			return true;
		} else {
			return false;
		}
